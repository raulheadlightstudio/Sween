...
const RegisterPhone = ({ navigation }) => {
  ...
  const handleNext = async () => {
    console.log(registerState.phone.length);
    if (registerState.phone === '' || registerState.phone.length < 10) {
    ...
    try {
      console.log('RP:', registerState.phone);
    ...
    if (e.length < 10) {
      ...
      dispatchRegister(
        RegisterActions.registerError(
          'Tu número móvil no puede quedar vacío o tener menos de 10 digitos'
        )
      );
    } else {
